...
public class TicketPool {
    ...
    private int ticketsReleased = 0;  // Total tickets released by all vendors
    private int ticketsSold = 0;        // Total tickets sold to customers
    private int maxCapacityReachedCount = 0; // Count of how many times max capacity was reached
    private int vendorWaitCount = 0;    // Count of vendors waiting to add tickets
    private int customerWaitCount = 0;  // Count of customers waiting to purchase tickets
    ...
    public synchronized void addTickets(int tickets, int vendorId) {
        ...
        if (availableTickets == maxCapacity) {
            maxCapacityReachedCount++;  // Increment when the pool reaches max capacity
        }

        notifyAll();  // Notify any waiting customers or vendors
    }
     ...
     */
    public int getMaxCapacityReachedCount() {
        return maxCapacityReachedCount;
    }
     ...
     */
    public int getCustomerWaitCount() {
        return customerWaitCount;
    }
     ...
     */
    public int getVendorWaitCount() {
        return vendorWaitCount;
    }
}
